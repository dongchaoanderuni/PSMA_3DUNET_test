...
config["data_file"] = os.path.abspath("../data/PSMA_data.h5")
config["model_file"] = os.path.abspath("../data/PSMA_segmentation_model.h5")
...
def fetch_training_data_files(folderName):
    ...
    for subject_dir in glob.glob(os.path.join(folderName, "*")):
        subject_files = list()
        for modality in (config["training_modalities"] + ["labels"]):
            subject_files.append(os.path.join(subject_dir, modality + ".nii.gz"))
        training_data_files.append(tuple(subject_files))
    return training_data_files

def main(folderName, overwrite=False):
    ...
    # print 'Scanning generator'
    # for i in range(n_validation_steps):
    #    data = validation_generator.next()
    #    print type(data), len(data), [j.shape for j in data]
    # return 
    # run training
